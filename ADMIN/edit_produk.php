...
if (isset($_POST['ubah'])) { // Jika user menceklis checkbox yang ada di form ubah, lakukan :
  ...
  if (move_uploaded_file($tmp, $path)) { // Cek apakah gambar berhasil diupload atau tidak
    ...
    if (is_file("img/" . $data['GAMBAR_LAPTOP'])) // Jika foto ada
      unlink("img/" . $data['GAMBAR_LAPTOP']); // Hapus file foto sebelumnya yang ada di folder images

    // Proses ubah data ke Database
    $query = "UPDATE laptop SET GAMBAR_LAPTOP='" . $fotobaru . "', NAMA_LAPTOP='" . $nama . "', PROCESSOR='" . $processor . "', RAM='" . $ram . "', HARDDISK='" . $harddisk . "', VGA='" . $vga . "', UKURAN_LAYAR='". $layar ."', SOUND_CARD='". $sound ."'  WHERE ID_LAPTOP='" . $id . "'";
    $sql = mysqli_query($conn, $query); // Eksekusi/ Jalankan query dari variabel $query
    ...
    echo "<script>alert('Data Gagal Disimpan karena gagal upload foto');document.location.href='data_produk.php'</script>\n";
  }
} else { // Jika user tidak menceklis checkbox yang ada di form ubah, lakukan :
  // Proses ubah data ke Database
  $query = "UPDATE laptop SET NAMA_LAPTOP='" . $nama . "', PROCESSOR='" . $processor . "', RAM='" . $ram . "', HARDDISK='" . $harddisk . "', VGA='" . $vga . "', UKURAN_LAYAR='". $layar ."', SOUND_CARD='". $sound ."'  WHERE ID_LAPTOP='" . $id . "'";
  $sql = mysqli_query($conn, $query); // Eksekusi/ Jalankan query dari variabel $query
