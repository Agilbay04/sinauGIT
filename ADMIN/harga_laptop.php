...
<head>
  ...
  <title>RIZQUINA Admin | Harga & Garansi</title>
              ...
              <form action="function_harga.php" class="user" method="post" enctype="multipart/form-data">
                <div class="input-group mb-3">
                  <select class="custom-select" id="inputGroupSelect01" name="laptop" require>
                    ...
                    <?php 
                      $sql = mysqli_query($conn, "SELECT * FROM det_laptop RIGHT JOIN laptop ON det_laptop.ID_LAPTOP = laptop.ID_LAPTOP");
                      while($row = mysqli_fetch_assoc($sql)) { 
                    ?>
                    <option value="<?= $row["ID_LAPTOP"]; ?>">[<?= $row["ID_DET_LAPTOP"]; ?>], <?= $row["NAMA_LAPTOP"]; ?></option>
                    <?php } ?>
                  </select>
                </div>
